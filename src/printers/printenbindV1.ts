...
/**
 * Print&Bind's legacy JSON API (`/v1/orders`, `/v1/delivery`, ...), kept as a
 * fallback for the REST integration in printenbindV2.ts. Which one handles
 * orders is an admin toggle read by the PrintEnBind facade (printenbind.ts);
 * nothing else should import this class directly.
 */
class PrintEnBindV1 {
  private static instance: PrintEnBindV1;
  ...
  // The hourly tracking / box-instruction crons live in the PrintEnBind
  // facade (printenbind.ts), which runs them on whichever API is active.
  private constructor() {}
  ...
  public static getInstance(): PrintEnBindV1 {
    if (!PrintEnBindV1.instance) {
      PrintEnBindV1.instance = new PrintEnBindV1();
    }
  ...
  > {
    ...
    if (orderComment) {
      ...
      const orderResp = await fetch(
        `${process.env['PRINTENBIND_V1_API_URL']}/v1/orders`,
        {
          method: 'POST',
          headers: {
      ...
      if (logging) {
        apiCalls.push({
          method: 'POST',
          url: `${process.env['PRINTENBIND_V1_API_URL']}/v1/orders`,
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      if (items[i].type == 'physical' && !physicalOrderCreated) {
        ...
        const response = await fetch(
          `${process.env['PRINTENBIND_V1_API_URL']}/v1/orders/articles`,
          {
            method: 'POST',
            headers: {
              Authorization: authToken!,
        ...
        if (logging) {
          apiCalls.push({
            method: 'POST',
            url: `${process.env['PRINTENBIND_V1_API_URL']}/v1/orders/articles`,
            body: articleBody,
        ...
        const articleResponse = await fetch(
          `${process.env['PRINTENBIND_V1_API_URL']}/v1/orders/${orderId}/articles`,
          {
            method: 'POST',
            headers: {
              Authorization: authToken!,
        ...
        if (logging) {
          apiCalls.push({
            method: 'POST',
            url: `${process.env['PRINTENBIND_V1_API_URL']}/v1/orders/${orderId}/articles`,
            body: articleBody,
    ...
    if (physicalOrderCreated) {
      ...
      const addDeliveryResult = await fetch(
        `${process.env['PRINTENBIND_V1_API_URL']}/v1/delivery/${orderId}`,
        {
          method: 'POST',
          headers: {
            Authorization: authToken!,
      ...
      if (logging) {
        apiCalls.push({
          method: 'POST',
          url: `${process.env['PRINTENBIND_V1_API_URL']}/v1/delivery/${orderId}`,
          body: deliveryData,
      ...
      const [orderResponse, deliveryResponse] = await Promise.all([
        fetch(`${process.env['PRINTENBIND_V1_API_URL']}/v1/orders/${orderId}`, {
          method: 'GET',
          headers: { Authorization: authToken! },
        }),
        fetch(`${process.env['PRINTENBIND_V1_API_URL']}/v1/delivery/${orderId}`, {
          method: 'GET',
          headers: { Authorization: authToken! },
        }),
      ]);
   ...
   */
  private async getAuthToken(_force: boolean = false): Promise<string | null> {
  ...
  > {
    try {
      ...
      const response = await fetch(
        `${process.env['PRINTENBIND_V1_API_URL']}/v1/orders/${orderId}/finish`,
        {
          method: 'POST',
          headers: {
            Authorization: authToken!,
      ...
      finishApiCalls.push({
        method: 'POST',
        url: `${process.env['PRINTENBIND_V1_API_URL']}/v1/orders/${orderId}/finish`,
        statusCode: response.status,
        responseBody,
      });
  ...
  private async setPaymentInfo(
    ...
    try {
      const orderResponse = await fetch(
        `${process.env['PRINTENBIND_V1_API_URL']}/v1/orders/${printApiOrderId}`,
        {
          method: 'GET',
          headers: { Authorization: authToken! },
        }
          ...
          const existingRecord = await this.prisma.shippingCostNew.findFirst({
            ...
            if (result.success) {
              const deliveryResponse = await fetch(
                `${process.env['PRINTENBIND_V1_API_URL']}/v1/delivery/${result.data.orderId}`,
                {
                  method: 'GET',
                  headers: { Authorization: authToken },
                }
  ...
  private async getOrderStatus(orderId: string): Promise<any> {
    try {
      const authToken = await this.getAuthToken();
      const response = await fetch(
        `${process.env['PRINTENBIND_V1_API_URL']}/v1/orders/${orderId}`,
        {
          method: 'GET',
          headers: { Authorization: authToken! },
        }
