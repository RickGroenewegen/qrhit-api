...
class WebSocketServer {
  ...
  constructor(server: HTTPServer) {
    ...
    if (!redisUrl) {
      ...
      this.logger.log('WebSocket server configured with Redis adapter for clustering');
    }
  ...
  private initializeHandlers() {
    this.io.on('connection', (socket: Socket) => {
      ...
      socket.on('rejoinGame', async (data: { gameId: string; playerName: string }) => {
  ...
  private async handleJoinGame(socket: Socket, data: JoinGameData) {
    try {
      ...
      console.log(`WebSocket: Player "${playerName}" attempting to join game "${gameId}" (isHost: ${isHost})`);
      ...
      if (player) {
        ...
        console.log(`WebSocket: Found existing player ${playerName} with ID ${player.id}`);
      } else {
        console.log(`WebSocket: Player ${playerName} not found in game data`);
      }
      
      console.log(`WebSocket: Current players in game ${gameId}:`, gameData.players.map(p => p.name));
      
      // Notify all players in the room
      this.io.to(gameId).emit('playerJoined', {
        players: gameData.players
      });
      console.log(`WebSocket: Notified all players in room ${gameId} about player join`);
  ...
  private async handleRejoinGame(socket: Socket, data: { gameId: string; playerName: string }) {
    try {
  ...
  private async handleRequestQuestion(socket: Socket, data: RequestQuestionData) {
    try {
  ...
  private async handleStartNextRound(socket: Socket, data: { gameId: string }) {
    try {
      ...
      this.io.to(gameId).emit('roundStarting', { round: gameData.currentRound });
    } catch (error) {
      this.logger.log('Error starting next round');
    }
  }

  private async handleShowFinalResults(socket: Socket, data: { gameId: string }) {
    try {
      const { gameId } = data;
