...
class Generator {
  ...
  public async generate(paymentId: string, ip: string, mollie: Mollie): Promise<void> {
    this.logger.log(blue.bold(`Starting generation for payment: ${white.bold(paymentId)}`));
    ...
    if (user.userId !== userId) {
      this.logger.log(color.red.bold('User is not the same as the one who made the payment'));
      return;
    }
    ...
    this.logger.log(color.green.bold(`Order processed successfully for payment: ${white.bold(paymentId)}`));
  }

  private async generatePDF(payment: any, playlist: any, ip: string): Promise<void> {
    let filename = '';
    let filenameDigital = '';

    const hash = crypto.createHmac('sha256', process.env['PLAYLIST_SECRET']!)
      .update(playlist.playlistId)
      .digest('hex');

    filename = sanitizeFilename(`${hash}_printer.pdf`.replace(/ /g, '_')).toLowerCase();
    filenameDigital = sanitizeFilename(`${hash}_digital.pdf`.replace(/ /g, '_')).toLowerCase();
    ...
    if (payment.orderType.name === 'digital') {
      ...
      try {
        ...
        exists = true;
        this.logger.log(color.yellow.bold(`Digital PDF already exists: ${color.white.bold(filenameDigital)}`));
      } catch (error) {
      ...
      try {
        ...
        exists = true;
        this.logger.log(color.yellow.bold(`Both PDFs already exist: ${color.white.bold(filename)} and ${color.white.bold(filenameDigital)}`));
      } catch (error) {
    ...
    this.pushover.sendMessage({
      title: `KA-CHING! € ${payment.orderType.amount.toString().replace('.', ',')} verdiend!`,
      message: `${orderName} heeft ${payment.numberOfTracks} ${cardType} kaarten besteld voor € ${payment.totalPrice.toString().replace('.', ',')}. Playlist: ${playlist.name}`,
      sound: 'incoming',
    }, ip);

    this.logger.log(blue.bold(`Retrieving tracks for playlist: ${white.bold(playlist.playlistId)}`));

    if (playlist.resetCache) {
      exists = false;
      this.logger.log(color.yellow.bold(`User has refreshed the playlist cache for playlist: ${white.bold(playlist.playlistId)} so we are regenerating the PDFs`));
    }
    ...
    if (!exists) {
      ...
      this.logger.log(blue.bold(`Storing ${white.bold(tracks.length)} tracks for playlist: ${white.bold(playlist.playlistId)}`));
      await this.data.storeTracks(playlist.id, tracks);

      this.logger.log(blue.bold(`Retrieving ${white.bold(tracks.length)} tracks for playlist: ${white.bold(playlist.playlistId)}`));
      const dbTracks = await this.data.getTracks(playlist.id);
      playlist.numberOfTracks = dbTracks.length;

      this.logger.log(blue.bold(`Creating QR codes for ${white.bold(tracks.length)} tracks for playlist: ${white.bold(playlist.playlistId)}`));
      ...
      for (const track of dbTracks) {
        ...
        const progress = Math.floor((tracks.indexOf(track) / tracks.length) * 20 + 70);
      }

      this.logger.log(blue.bold(`Creating PDF tracks for playlist: ${white.bold(playlist.playlistId)}`));

      const [generatedFilenameDigital, generatedFilename] = await Promise.all([
        this.pdf.generatePDF(filenameDigital, playlist, payment, 'digital', 80, 89),
    ...
    if (payment.orderType.name != 'digital') {
      payment.printerPageCount = await this.utils.countPdfPages(`${process.env['PUBLIC_DIR']}/pdf/${filename}`);
    ...
    await this.mail.sendEmail(payment.orderType.name, payment, playlist, filename, filenameDigital);

    this.logger.log(color.green.bold(`PDF generated successfully for playlist: ${white.bold(playlist.playlistId)}`));
  }
}
